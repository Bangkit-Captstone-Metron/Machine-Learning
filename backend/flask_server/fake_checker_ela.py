...
def convert_to_ela_image(image):
    ...
    im.save(resaved_filename, 'JPEG', quality=100)
...
def process_image(image):
    ...
    return {'fake': bool(np.argmax(y_pred, axis=1)[0]), 'confidence': f'{np.amax(y_pred) * 100:0.2f}'}
...
def _get_image_file(image_file):
    ela_image = np.array(convert_to_ela_image(
        image_file).resize((128, 128))).flatten() / 255.0
...
def _get_image(url, session=session):
    ...
    ela_image = np.array(convert_to_ela_image(
        original_image).resize((128, 128))).flatten() / 255.0
    ela_image = ela_image.reshape(-1, 128, 128, 3)
